...
# Filtering to only keep observations that correspond to low wind (wind < q20(wind_all_records) & high radiation (ens>q90(ens_day_records)))
# records.df <- subset_data(
#   data.df = bind_rows(records_pameseb.df, records_irm_df),
#   sensor.chr = "tsa",
#   dateRange.chr = c(as.Date("2015-11-01"),as.Date("2017-11-01")),
#   filter.chr ="high_rad_low_wind"
# )
# cat(paste0("Your dataset contains ", nrow(records.df)/2, " hourly records"))
...
records.df <- is_it_day(records.df)
colnames(records.df)
...
ens.scatter.plot <- build_plot(input_records.df = make_wide(records.df, "ens"), sensor_name.chr = "ens", plot.chr = "scatter")
vvt.scatter.plot <- build_plot(input_records.df = make_wide(records.df, "vvt"), sensor_name.chr = "vvt", plot.chr = "scatter")
tsa.scatter.plot <- build_plot(input_records.df = make_wide(records.df, "tsa"), sensor_name.chr = "tsa", plot.chr = "scatter")

# Build the time serie plot for each sensors
ens.time.plot <- build_plot(input_records.df = records.df, sensor_name.chr = "ens", plot.chr = "timeSerie")
vvt.time.plot <- build_plot(input_records.df = records.df, sensor_name.chr = "vvt", plot.chr = "timeSerie")
tsa.time.plot <- build_plot(input_records.df = records.df, sensor_name.chr = "tsa", plot.chr = "timeSerie")
...
bland_altman.plot <- compute_ba(input_records_wide.df= make_wide(records.df, "tsa"), output="plot")
bland_altman.stats.df<- bland.altman.stats(
  (make_wide(records.df, "tsa"))[3][[1]], # pameseb61
  (make_wide(records.df, "tsa"))[2][[1]]  # irm1000
)
...
colnames(tsa_wide.df) <- c("mtime", "tsa_1000", "tsa_61")
...
is_it_day_wide.df <- make_wide(records.df, "day")
...
tsa_tsa_corr.time.plot <- build_plot(tsa_corr_long.df , plot.chr= "timeSerie", "tsa")
ggplotly(tsa_tsa_corr.time.plot)

# plotting the new Bland-Altman
colnames(tsa_corr_long.df) <- c("mtime", "sid", "tsa")
bland_altman.corr.plot <- compute_ba(input_records_wide.df= make_wide(tsa_corr_long.df, "tsa"), output="plot")
bland_altman.stats.df<- bland.altman.stats(
  (make_wide(records.df, "tsa"))[3][[1]], # pameseb61
  (make_wide(records.df, "tsa"))[2][[1]]  # irm1000
)
