...
* For night_only records, the scatter still presents a trend going from positive bias to __negative bias__ ==> the negative differences also observable at __night__
...
* For daily_max +/- 2h records, the scatter still presents a negative trend but __no more positive bias__ (Pameseb is always warmer)
...
* For the __daily_max +/- 2h__ approach, the learner that performs the best is the Fast [k-Nearest Neighbor](https://www.youtube.com/watch?v=3lp5CmSwrHI) with a mse of `r getBMRAggrPerformances(hci.bmr.l)$regr.all.hci$regr.fnn`. By definition it tries to approximate based on some __local vicinity__ (peaks).
